...
public:
    ...
    virtual const AbstractValue& operator=( const AbstractValue& pValue ) throw (dab::Exception) = 0;
    ...
    template <class Type> operator Type() const throw (dab::Exception);
    template <class Type> void get( Type& pValue ) const throw (dab::Exception);
    template <class Type> Type& get() throw (dab::Exception);
    template <class Type> const Type& get() const throw (dab::Exception);
    
    template <class Type> void set(const Type& pType) const throw (dab::Exception);
...
public:
    ...
    const AbstractValue& operator=( const AbstractValue& pValue ) throw (dab::Exception);
...
template <class Type>
AbstractValue::operator Type() const throw (dab::Exception)
{
...
template <class Type>
void
AbstractValue::get( Type& pValue ) const throw (dab::Exception)
{
    if( typeMatch<Type>() == false ) throw( dab::Exception( "TYPE ERROR: supplied type " + std::string(std::type_index(typeid(Type)).name()) + " doesn't match stored type " + std::string(mType.name()), __FILE__, __FUNCTION__, __LINE__ ));
...
template <class Type>
Type&
AbstractValue::get() throw (dab::Exception)
{
    if( typeMatch<Type>() == false ) throw( dab::Exception( "TYPE ERROR: supplied type " + std::string(std::type_index(typeid(Type)).name()) + " doesn't match stored type " + std::string(mType.name()), __FILE__, __FUNCTION__, __LINE__ ));
...
template <class Type>
const Type&
AbstractValue::get() const throw (dab::Exception)
{
    if( typeMatch<Type>() == false ) throw( dab::Exception( "TYPE ERROR: supplied type " + std::string(std::type_index(typeid(Type)).name()) + " doesn't match stored type " + std::string(mType.name()), __FILE__, __FUNCTION__, __LINE__ ));
...
template <class Type>
void
AbstractValue::set(const Type& pType) const throw (dab::Exception)
{
    if( typeMatch<Type>() == false ) throw( dab::Exception( "TYPE ERROR: supplied type " + std::string(std::type_index(typeid(Type)).name()) + " doesn't match stored type " + std::string(mType.name()), __FILE__, __FUNCTION__, __LINE__ ));
...
template <class Type>
const AbstractValue&
Value<Type>::operator=( const AbstractValue& pValue ) throw (dab::Exception)
{
...
template< class Type, int Dim >
const AbstractValue&
Value< std::array<Type,Dim> >::operator=( const AbstractValue& pValue ) throw (dab::Exception)
{
